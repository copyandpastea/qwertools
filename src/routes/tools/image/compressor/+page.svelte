<script lang="ts">
  ...
  import { compressImage, type CompressOptions } from "$lib/features/image-compressor";
...
<div class="mx-auto px-4 py-6">
  ...
  <p class="text-sm text-zinc-600 mb-6">
    Compress your images to reduce file size while maintaining quality. Supports PNG, JPG, WebP, and GIF formats.
  </p>
    ...
    {#if originalFile}
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="wiki-field">
          ...
          />
          <div class="wiki-help">Target maximum file size after compression</div>
        </div>

        <div class="wiki-field">
          <label for="max-dimension" class="wiki-label">Max Width/Height (px)</label>
          <input
            id="max-dimension"
            type="number"
            class="wiki-input"
            ...
            disabled={isCompressing}
          />
          <div class="wiki-help">Maximum dimension (width or height). Set to 0 to keep original size.</div>
        </div>
        ...
        <div class="wiki-field">
          ...
          <input
            ...
            disabled={isCompressing}
          />
          <div class="wiki-help">Initial compression quality (0.92 = 92%)</div>
        </div>

        <div class="wiki-field">
          <label for="output-format" class="wiki-label">Output Format</label>
          <select id="output-format" class="wiki-select" bind:value={fileType} disabled={isCompressing}>
      ...
      {#if compressError}
        <div class="mt-4 wiki-notice wiki-notice-danger">
          <b>Compression Error:</b> {compressError}
        </div>
      {/if}
      </div>
    {/if}
  ...
  {#if compressedFile && compressedUrl}
    <div class="wiki-card">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <!-- Compressed Image -->
        <div>
          <h3 class="text-sm font-semibold text-zinc-800 mb-2">Compressed Image</h3>
          <div class="wiki-thumb">
            <img src={compressedUrl} alt="Compressed" class="max-h-64 w-full object-contain" />
            <div class="mt-2 flex flex-wrap items-center gap-2">
              <span class="wiki-badge">{compressedFile.type || "unknown type"}</span>
          ...
          {#if compressedFile}
            <div class="mt-2 text-xs text-zinc-600">
              ...
              <div class="text-green-700 font-semibold mt-1">
                Saved: {formatFileSize(originalFile!.size - compressedFile.size)} ({getCompressionRatio().toFixed(1)}%)
              </div>
            </div>
          {/if}
        </div>
